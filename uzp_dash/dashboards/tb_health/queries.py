...
# Задачи с текстом по организациям, у которых отток уже случился.
#
...
# (~12 млн строк). Берём только организации с фактическим оттоком (:inns), только
# задачи с текстом и только за окно самого оттока плюс текущий месяц: задачу на
# уход заводят в месяц ухода или следующим отчётным.
#
# Тип задачи НЕ фильтруем, но возвращаем признаком. Причина ухода нередко написана
# в задаче другого типа — контактной или по привлечению, заведённой ровно тогда,
# когда люди уходили; если брать только задачи типа «Отток», у половины крупнейших
# потерь причина остаётся неизвестной. А вот ОБЕЩАНИЕ возврата читается только из
# задач по оттоку (см. bank._outflow_promises): в остальных «вернутся» с тем же
# успехом относится к расширению.
OUTFLOW_TASK_TEXT = """
WITH gmap AS (""" + _GMAP + """)
SELECT g.new_gosb_id, f.tb_id, f.inn, f.task_create_dt,
       (f.task_type = 'Отток')                  AS is_outflow_task,
