...
def _outflow_facts(a, al, unit: str) -> str:
    ...
    top = "; ".join(
        ...
        f'−{r["kept"]:.0f} чел, '
        + ("отработка не подтверждена" if r["known"] and not r["tasks"]
    ...
    return (
        ...
        f"Раздел разделён на две группы клиентов. Первая — отработка проведена, "
        f"возврат не состоялся ({o['kept_worked']:.0f} чел). Вторая — отток на "
        f"контроль, где отработка не подтверждена ({o['kept_silent']:.0f} чел).\n"
...
def _outflow_fallback(a) -> str:
    ...
    if o["n_silent"]:
        parts.append(f"Потери на {o['kept_silent']:.0f} чел отнесены в группу "
                     f"«Отток на контроль»: отработка по ним не подтверждена, "
                     f"с них и рекомендуется начинать.")
    if o["n_worked"]:
