...
def section_narratives(ctx, a) -> dict:
    ...
    prompt = (
        "Ты руководитель продаж зарплатных проектов. Пишешь управляющему банком "
        "короткие замечания к разделам отчёта — так, как пишут живому человеку, "
        "который сам видит цифры на экране.\n\n"
        ...
        "• matrix — где именно сидит недобор: конкретные единицы и сегменты, есть ли "
        "общая для всех слабая точка.\n"
        "• gosb — за какие подразделения браться первыми и почему именно за них; "
        "если у подразделения не хватает своих организаций, сказать это прямо.\n"
        "• orgs — с чего начать работу по списку организаций.\n"
        "• outflow — ушедшие люди: что видно по отработке и по комментариям "
        "сотрудников. Если по крупным потерям задач не заводили — это главное, "
        "что нужно сказать.\n\n"
        "КАК ПИСАТЬ — это важнее содержания:\n"
        "— 2–3 предложения на блок. Короткие фразы. Обычные слова.\n"
        "— Максимум два числа на блок, и только те, из-за которых меняется решение. "
        "Остальные цифры управляющий и так видит в таблице над твоим текстом.\n"
        "— Начинай с сути, а не с преамбулы. Первое предложение — что не так "
        "(или что хорошо), второе — что с этим делать.\n"
        "— Пиши утвердительно и конкретно: «взяться за X», «там не работали "
        "с Y организациями», а не «рекомендуется обратить внимание».\n\n"
        "ЗАПРЕЩЕНО (текст с этим считается негодным):\n"
        "— Пересказывать методику расчёта и объяснять, откуда взялись числа: "
        "как считается прогноз, что с чем складывается или не складывается, "
        "что месяц не закрыт, чем витрина отличается от факта. Управляющему нужен "
        "вывод, а не устройство отчёта.\n"
        "— Повторять своими словами эту инструкцию.\n"
        "— Канцелярит и штампы: «важно отметить», «стоит отметить», «таким образом», "
        "«в целом», «следует учитывать», «ключевой фактор», «во втором эшелоне», "
        "«вторичны по приоритету», «требует пристального внимания», «необходимо "
        "усилить работу», «драйвер роста», «точка роста».\n"
        "— Перечислять подряд пять-шесть единиц с процентами. Назови одну-две "
        "главные, остальные сверни в «и ещё N».\n"
        "— Повторять в разных блоках одно и то же число или одну и ту же мысль.\n"
        "— Начинать все блоки одинаково.\n\n"
