...
def _outflow_facts(a, al, unit: str) -> str:
    ...
    top = "; ".join(
        ...
        f'−{r["kept"]:.0f} чел, '
        + ("задач не заводили" if r["known"] and not r["tasks"]
           else "отток отработан" if r["worked"]
           else f'задач {r["tasks"]}, результата нет' if r["tasks"]
           else "месяц ухода вне окна задач")
        for r in o["rows"][:5])
    return (
        ...
        f"Отработка потерь: успешно закрытая задача по оттоку есть у {o['n_worked']} "
        f"организаций ({o['kept_worked']:.0f} чел); НИ ОДНОЙ задачи не заводили по "
        f"{o['n_silent']} организациям ({o['kept_silent']:.0f} чел); ещё по "
        f"{o['n_unknown']} месяц ухода не попал в окно воронки.\n"
...
def section_narratives(ctx, a) -> dict:
    ...
    prompt = (
        ...
        "• outflow — потери портфеля: что показывает отработка оттока и комментарии "
        "сотрудников. Организации, по которым задачи не заводились, ставь на первое "
        "место.\n\n"
        ...
        "ломает термин: «задачи не заводились» вместо «отсутствие заведения задач».\n\n"
        "ОБЪЁМ И РИТМ:\n"
...
def _outflow_fallback(a) -> str:
    ...
    parts = [f"Ушли {o['tot_gone']:.0f} человек, вернулись {o['tot_ret']:.0f}. "
             f"Потеряны окончательно {o['tot_kept']:.0f}."]
    if o["n_silent"]:
        parts.append(f"По {o['n_silent']} организациям ({o['kept_silent']:.0f} чел) "
                     f"задач не заводили вовсе — начинать нужно с них.")
    if o["n_worked"]:
        parts.append(f"Отток отработан у {o['n_worked']} организаций "
                     f"({o['kept_worked']:.0f} чел): там люди не вернулись, "
                     f"несмотря на закрытые задачи.")
    return " ".join(parts)
