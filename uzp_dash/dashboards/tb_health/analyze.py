    ...
    for _, o in cand.iterrows():
        ...
        if o.get("lever") == "Вернуть":
            ...
            if bool(o.get("out_worked", False)):
                ...
                work_rows.append((o, f"Отток {months}: отработка не подтверждена — "
                                     f"начать работу по возврату"))
            elif n_o == 0:
                work_rows.append((o, f"Отток {months}: задач {n_t}, причина ухода "
                                     f"не отработана"))
            else:
...
def _yoy_groups(rows: list, nid: int, fmonths: dict, fwindow: set) -> list:
    ...
    kinds = [
        ...
        ("missed", "Отток без подтверждённой отработки",
         "в месяцы оттока активность по клиенту в воронке не отражена — требует разбора"),
